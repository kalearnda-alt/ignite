...
function getAuthErrorMessage(error, action = 'sign_in') {
  if (!(error instanceof FirebaseError)) {
    return action === 'sign_up'
      ? 'Unable to create the admin account right now. Please try again.'
      : 'Unable to sign in right now. Please try again.'
  }

  const authMessages = {
    'auth/email-already-in-use': 'This admin account already exists. Sign in instead.',
    'auth/invalid-email': 'Enter a valid email address.',
    'auth/user-not-found': 'No admin account was found for these credentials.',
